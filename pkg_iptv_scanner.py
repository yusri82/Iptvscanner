...
├─◉ """+tokenr+str(mac)+"""  \33[0m\33[1;4;94mCPM➢"""+str(cpm)+""" \33[0m 
...
├⟢StbType➢ """+stb_type+"""
├⟢Country➢ """+country+"""
├⟢SettingsPass➢ """+settings_password+"""
╰─⟢Comment➢ """+comment+""" """
		imza="""
╭─➤🅸🅿🆃🆅 💎 🆂🅲🅰🅽🅽🅴🆁
├●Scan Date➤"""+str(time.strftime('%d-%m-%Y'))+"""
├●Scan Time➤"""+str(time.strftime('%H:%M:%S'))+"""
├◉𝗥𝗲𝗮𝗹 ➤ """+str(reall)+"""
├◉𝗣𝗮𝗻𝗲𝗹➤ http://"""+str(panell)+"""/c/
├◉𝗠𝗮𝗰 ➤ """+str(mac)+"""
├◉𝗘𝘅𝗽➤ """+str(trh)+"""
╰─◉𝗛𝗶𝘁 𝗕𝘆 """+str(nickn)+"""
╭➤🄺🄰🄽🄰🄻♻️🄲🄷🄴🄲🄺
...
ses=requests.Session()
Dosyab="/sdcard/hits/🅿🅺🅶." + panel.replace(":","_").replace('/','') +".txt"
